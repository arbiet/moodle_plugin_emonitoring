...
//
// E-Monitoring is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// E-Monitoring is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with E-Monitoring.  If not, see <http://www.gnu.org/licenses/>.

/**
 * E-Monitoring block renderer.
 *
 * @package    block_e_monitoring
...
class renderer extends plugin_renderer_base {
    ...
    public function render_completion_progress(completion_progress $progress) {
        ...
        // Add the percentage below the block title.
        if ($showpercentage && !$simple) {
            $progressValue = $progress->get_percentage();
            $percentageContent = get_string('progress', 'block_completion_progress') . ': ' . $progressValue . '%';
        ...
        foreach ($activities as $activity) {
            ...
            // Define the content for the progress bar cell.
            $progressbarcontent = html_writer::div('', 'progress-bar');
            
            // Define the activity details including the progress bar cell.
            $activitydetails = html_writer::start_div('activity-details');
            //$activitydetails .= html_writer::div($shortened_name, 'activity-name');
            $max_length = 7; // Panjang maksimum sebelum dipotong
$shortened_name = (strlen($activity->name) > $max_length) ? substr($activity->name, 0, $max_length) . '...' : $activity->name;
$activitydetails .= html_writer::div($shortened_name, 'activity-name');

	    if ($complete == COMPLETION_COMPLETE || $complete == COMPLETION_COMPLETE_PASS) {
                $activitydetails .= html_writer::div(get_string('completed', 'completion'), 'completion-status completed');
            } else {
                $activitydetails .= html_writer::div(get_string('not_completed', 'block_completion_progress'), 'completion-status not-completed');
            }
            $activitydetails .= html_writer::div($progressbarcontent, null, $celloptions);
            $activitydetails .= html_writer::end_div(); // end of activity-details
        
            // Add the activity details to the content.
            $content .= html_writer::div($activitydetails, 'progress-bar-row');
        }
    ...
    public function render_completion_progress_simple(completion_progress $progress) {
        global $CFG, $USER;
