...
class overview extends \table_sql {
    ...
    public function __construct(completion_progress $progress, $groups, $roleid, $bulkoperations) {
        ...
        $tablecolumns[] = 'kelompok';
        $tableheaders[] = get_string('kelompok', 'block_completion_progress');
        ...
        // Tambahkan kolom baru untuk nilai berdasarkan tugas yang dipilih
        $grade_items = $this->get_grade_items();
        usort($grade_items, function($a, $b) {
            return strcmp($a->itemname, $b->itemname);
        });
        
        // Tambahkan kolom untuk grade item tertentu berdasarkan parameter
        $selected_gradeitemid = optional_param('gradeitemid', 0, PARAM_INT);
        foreach ($grade_items as $grade_item) {
            if ($selected_gradeitemid > 0 && $grade_item->id == $selected_gradeitemid) {
                $tablecolumns[] = 'grade_' . $grade_item->id;
                // $tableheaders[] = $grade_item->itemname;
                $tableheaders[] = 'Grade<br><small>'.$grade_item->itemname.'<small>';
            }
        }
        ...
        $this->column_class('kelompok', 'col-kelompok');
        ...
        foreach ($grade_items as $grade_item) {
            if ($selected_gradeitemid == 0 || $grade_item->id == $selected_gradeitemid) {
                $this->column_class('grade_' . $grade_item->id, 'col-grade');
            }
        }
        ...
        // Modifikasi query SQL untuk mengambil nilai siswa berdasarkan tugas yang dipilih
        $fields = "$picturefields, l.timeaccess, g.name AS kelompok";
        foreach ($grade_items as $grade_item) {
            if ($selected_gradeitemid == 0 || $grade_item->id == $selected_gradeitemid) {
                $fields .= ", (SELECT gg.finalgrade FROM {grade_grades} gg WHERE gg.itemid = {$grade_item->id} AND gg.userid = u.id) AS grade_{$grade_item->id}";
            }
        }
    ...
    public function col_kelompok($row) {
        return $row->kelompok;
    }

    public function get_grade_items() {
        global $DB;
        $sql = "SELECT id, itemname FROM {grade_items} WHERE courseid = :courseid AND itemtype = 'mod'";
        return $DB->get_records_sql($sql, ['courseid' => $this->progress->get_course()->id]);
    }

    public function __call($name, $arguments) {
        if (strpos($name, 'col_grade_') === 0) {
            $grade_item_id = substr($name, 10);
            return isset($arguments[0]->{"grade_$grade_item_id"}) ? round($arguments[0]->{"grade_$grade_item_id"}, 2) : '-';
        }
        throw new \coding_exception("Call to undefined method " . get_class($this) . "::{$name}()");
    }
    ...
    public function col_progressbar($row) {
        return $this->output->render_completion_progress_simple($this->progress);
    }
